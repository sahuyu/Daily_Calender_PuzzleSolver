...
%% Pieces shape
PiecesShape_DailyPuzzle_withWeekDay_v1;

%% Frame shape
FrameDefault = false(8,7);
FrameDefault(1:2,7)=true(2,1);FrameDefault(8,1:4)=true(1,4);
...
FrameQuestion = dailyPuzzle_withWeekDay(Month,Day,Week);
                    ...
                    for L6=1:LN
                        ...
                        for L7=1:LN
                            ...
                            for L8=1:LN
                                ...
                                Frame8 = newFrame7;
                                [isfit8, newFrame8] = isFit(Frame8, ExpandedPuzzlePieces{8,L8}, ListOfPossibleSize{9});
                                if isfit8
                                Ouptput8 = Ouptput7;
                                Ouptput8(8) = L8;
                                for L9=1:LN
                                    if ExpandedPuzzlePiecesFlag(9,L9)
                                    Frame9 = newFrame8;
                                    [isfit9, newFrame9] = isFit(Frame9, ExpandedPuzzlePieces{9,L9}, ListOfPossibleSize{10});
                                    if isfit9
                                    Ouptput9 = Ouptput8;
                                    Ouptput9(9) = L9;
                                    for L10=1:LN
                                        if ExpandedPuzzlePiecesFlag(10,L10)
                                        Frame10 = newFrame9;
                                        [isfit10, newFrame10] = isFit(Frame10, ExpandedPuzzlePieces{10,L10}, []);
                                        if isfit10
                                            Ouptput10 = Ouptput9;
                                            Ouptput10(10) = L10;
                                            write(filename,Ouptput10);
                                        end
                                        end
                                    end
                                    end
...
%% Plotting
ColorMaps = [1 1 1; 0 0.4 0.8; 0.4 0 0.8; 1 0.6 0.2; 1 1 0; 1 0 0; 0.2 0.6 1; 0.57 0.8 0.31; 0 0.69 0.31; 1 0.75 0.76; 0.7 0.7 0.7];
NRow = 4;
NColumn = 7;
NImage = NRow*NColumn;
...
for p=1:NPage
    ...
    titlename = ['Solution of ', date2String(Month,Day), '(', Week2String(Week,:), ')', ' (Page', num2str(p,'%03d'), ')'];
    title(H0, titlename);
    set(gcf,'Position',[160 80 1480 880]);
    %H0.Visible = 'off';
    orient(gcf,'landscape')
    PrintFileName{p,1} = ['output/Solution_', date2String(Month,Day), '(', Week2String(Week,:), ')', '_P', num2str(p,'%03d'), '.pdf'];
...
function [frame_question] = dailyPuzzle_withWeekDay(Month,Day,Week)
    frame_question = false(8,7);
    ...
    switch Week
        case 1
            frame_question(7,5) = true;
        case 2
            frame_question(7,6) = true;
        case 3
            frame_question(7,7) = true;
        case 4
            frame_question(8,5) = true;
        case 5
            frame_question(8,6) = true;
        case 6
            frame_question(8,7) = true;
        case 7
            frame_question(7,4) = true;
    end
end