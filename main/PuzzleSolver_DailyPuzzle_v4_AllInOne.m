...
%% Pieces shape
PiecesShape_DailyPuzzle_v4;

%% Frame shape
FrameDefault = false(7,7);
FrameDefault(1:2,1)=true;FrameDefault(3,1:2)=true;FrameDefault(6:7,7)=true;
...
%% Qusetion
FrameQuestion = dailyPuzzle_v4(Month,Day);
...
for p=1:NPage
    ...
    titlename = ['Solution of ', date2String(Month,Day), ' (Page', num2str(p,'%03d'), ')'];
    title(H0, titlename);
    set(gcf,'Position',[100 100 1520 820]);
    %H0.Visible = 'off';
    orient(gcf,'landscape')
    PrintFileName{p,1} = ['output/Solution_', num2str(Month,'%02d'), num2str(Day,'%02d'), '_P', num2str(p,'%03d'), '.pdf'];
...
function [frame_question] = dailyPuzzle_v4(Month,Day)
    ...
    if Day<=25
        ...
        frame_question(ceil((Day+2)/7)+2,remDay) = true;
    else
        remDay = Day-25;
        frame_question(7,remDay) = true;
    end
end